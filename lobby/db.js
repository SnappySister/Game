...
addColumnIfMissing('accounts', 'is_admin', "INTEGER NOT NULL DEFAULT 0");    // 管理员(0=否,1=是)
...
/* ==================== 游戏配置 game_config ==================== */
const stmtGetConfig = db.prepare('SELECT value_json FROM game_config WHERE game_type = ? AND key = ?');
const stmtSetConfig = db.prepare('INSERT OR REPLACE INTO game_config (game_type, key, value_json) VALUES (?, ?, ?)');
const stmtGetAllConfig = db.prepare('SELECT key, value_json FROM game_config WHERE game_type = ?');
const stmtDeleteConfig = db.prepare('DELETE FROM game_config WHERE game_type = ? AND key = ?');

// 读单个配置项(返回解析后的值或null)
function getConfig(gameType, key) {
  const row = stmtGetConfig.get(gameType, key);
  if (!row) return null;
  try { return JSON.parse(row.value_json); } catch (e) { return null; }
}

// 写配置项(值会被JSON序列化)
function setConfig(gameType, key, value) {
  stmtSetConfig.run(gameType, key, JSON.stringify(value));
}

// 读某游戏所有配置(返回 {key: value} 对象)
function getAllConfig(gameType) {
  const rows = stmtGetAllConfig.all(gameType);
  const result = {};
...
// 删除单个配置项(恢复默认)
function deleteConfig(gameType, key) {
  stmtDeleteConfig.run(gameType, key);
}

// 删除某游戏所有配置(全部恢复默认)
function clearConfig(gameType) {
  db.prepare('DELETE FROM game_config WHERE game_type = ?').run(gameType);
}

/* ==================== 管理员 ==================== */
const stmtSetAdmin = db.prepare('UPDATE accounts SET is_admin = ? WHERE id = ?');

function setAdmin(accountId, isAdmin) {
  stmtSetAdmin.run(isAdmin ? 1 : 0, accountId);
  return getAccountById(accountId);
}
